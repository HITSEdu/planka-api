...
func New(cfg config.Config, db *sql.DB) *http.Server {
	...
	friendHandler := handlers.NewFriends(db)
	...
	mux.HandleFunc("PATCH /api/Profile", authHandler.APIUpdateProfile)
	...
	mux.HandleFunc("GET /friends", friendHandler.List)
	mux.HandleFunc("POST /friends/requests", friendHandler.CreateRequest)
	mux.HandleFunc("POST /friends/requests/{id}/accept", friendHandler.AcceptRequest)
	mux.HandleFunc("DELETE /friends/requests/{id}", friendHandler.DeleteRequest)
	mux.HandleFunc("DELETE /friends/{id}", friendHandler.Remove)
	mux.HandleFunc("GET /friends/{id}/events", friendHandler.ListSharedEvents)
